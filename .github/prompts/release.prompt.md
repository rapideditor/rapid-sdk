...
3. **Look up PR numbers** by fetching:
   ```
   https://github.com/rapideditor/rapid-sdk/pulls?q=is%3Apr+is%3Aclosed
   ```
...
4. **Update `CHANGELOG.md`** — insert a new section immediately above the previous release heading. Follow the existing format exactly:
   ...
   - Add reference links at the bottom of the section:
     - PR links: `[#NNN]: https://github.com/rapideditor/rapid-sdk/issues/NNN`

5. **Update versions** — this is a monorepo with workspaces. Set `"version"` to `"${input:version}"` in the root `package.json` and in each workspace `package.json` under `packages/`.
