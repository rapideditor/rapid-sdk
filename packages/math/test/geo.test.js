...
describe('math/geo', () => {
  describe('geoLatToMeters', () => {
    it('0 degrees latitude is 0 meters', () => {
      assert.equal(math.geoLatToMeters(0), 0);
    });

    it('1 degree latitude is approx 111 km', () => {
      assert.closeTo(math.geoLatToMeters(1), 110946, 1);
    });
  ...
  describe('geoLonToMeters', () => {
    it('0 degrees longitude is 0 km', () => {
      assert.equal(math.geoLonToMeters(0, 0), 0);
    });

    it('distance of 1 degree longitude varies with latitude', () => {
      assert.closeTo(math.geoLonToMeters(1, 0),  111319, 1);
      assert.closeTo(math.geoLonToMeters(1, 15), 107526, 1);
      assert.closeTo(math.geoLonToMeters(1, 30), 96406, 1);
      assert.closeTo(math.geoLonToMeters(1, 45), 78715, 1);
      assert.closeTo(math.geoLonToMeters(1, 60), 55660, 1);
      assert.closeTo(math.geoLonToMeters(1, 75), 28812, 1);
      assert.equal(math.geoLonToMeters(1, 90), 0);
    });
  ...
  describe('geoMetersToLat', () => {
    it('0 meters is 0 degrees latitude', () => {
      assert.equal(math.geoMetersToLat(0), 0);
    });
    it('111 km is approx 1 degree latitude', () => {
      assert.closeTo(math.geoMetersToLat(110946), 1, 1e-5);
    });
    it('-111 km is approx -1 degree latitude', () => {
      assert.closeTo(math.geoMetersToLat(-110946), -1, 1e-5);
    });
  ...
  describe('geoMetersToLon', () => {
    it('0 meters is 0 degrees longitude', () => {
      assert.equal(math.geoMetersToLon(0, 0), 0);
    });

    it('distance of 1 degree longitude varies with latitude', () => {
      assert.closeTo(math.geoMetersToLon(111319, 0), 1, 1e-4);
      assert.closeTo(math.geoMetersToLon(107526, 15), 1, 1e-4);
      assert.closeTo(math.geoMetersToLon(96405, 30), 1, 1e-4);
      assert.closeTo(math.geoMetersToLon(78715, 45), 1, 1e-4);
      assert.closeTo(math.geoMetersToLon(55660, 60), 1, 1e-4);
      assert.closeTo(math.geoMetersToLon(28812, 75), 1, 1e-4);
      assert.equal(math.geoMetersToLon(1, 90), 0);
      assert.equal(math.geoMetersToLon(1, 91), 0);
    });
  ...
  describe('geoSphericalDistance', () => {
    it('distance between two same points is zero', () => {
      const a = [0, 0];
      const b = [0, 0];
      assert.equal(math.geoSphericalDistance(a, b), 0);
    });
    it('a straight 1 degree line at the equator is aproximately 111 km', () => {
      const a = [0, 0];
      const b = [1, 0];
      assert.closeTo(math.geoSphericalDistance(a, b), 111319, 1);
    });
    it('a pythagorean triangle is (nearly) right', () => {
      const a = [0, 0];
      const b = [4, 3];
      assert.closeTo(math.geoSphericalDistance(a, b), 555804, 1);
    });
    it('east-west distances at high latitude are shorter', () => {
      const a = [0, 60];
      const b = [1, 60];
      assert.closeTo(math.geoSphericalDistance(a, b), 55660, 1);
    });
  ...
  describe('geoZoomToScale', () => {
    it('converts from zoom to projection scale (tileSize = 256)', () => {
      assert.closeTo(math.geoZoomToScale(17), 5340353.715440872);
    });

    it('converts from zoom to projection scale (tileSize = 512)', () => {
      assert.closeTo(math.geoZoomToScale(17, 512), 10680707.430881744);
    });
  });

  describe('geoScaleToZoom', () => {
    it('converts from projection scale to zoom (tileSize = 256)', () => {
      assert.closeTo(math.geoScaleToZoom(5340353.715440872), 17);
    });

    it('converts from projection scale to zoom (tileSize = 512)', () => {
      assert.closeTo(math.geoScaleToZoom(10680707.430881744, 512), 17);
    });
  });

  describe('geoSphericalClosestPoint', () => {
    it('returns null if no points', () => {
      assert.equal(math.geoSphericalClosestPoint([], [1, 1]), null);
    });
