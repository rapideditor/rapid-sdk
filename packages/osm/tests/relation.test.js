describe('iD.osmRelation', function () {
  ...
  describe('#geometry', function () {
    it("returns 'area' for multipolygons", function () {
      expect(iD.osmRelation({ tags: { type: 'multipolygon' } }).geometry(iD.coreGraph())).to.equal(
        'area'
      );
    });
  ...
  describe('#isDegenerate', function () {
    ...
    it('returns false for a relation with members', function () {
      expect(iD.osmRelation({ members: [{ id: 'a', role: 'inner' }] }).isDegenerate()).to.equal(
        false
      );
    });
  });
  ...
  describe('#replaceMember', function () {
    ...
    it("replaces a member which doesn't already exist", function () {
      ...
      expect(r.replaceMember({ id: 'a' }, { id: 'b', type: 'node' }).members).to.eql([
        { id: 'b', role: 'a', type: 'node' }
      ]);
    });
    ...
    it('uses the replacement type', function () {
      var r = iD.osmRelation({ members: [{ id: 'a', role: 'a', type: 'node' }] });
      expect(r.replaceMember({ id: 'a' }, { id: 'b', type: 'way' }).members).to.eql([
        { id: 'b', role: 'a', type: 'way' }
      ]);
    });
    ...
    it('keeps duplicate members if `keepDuplicates = true`', function () {
      ...
      expect(r.replaceMember({ id: 'a' }, { id: 'b', type: 'node' }, true).members).to.eql([
        { id: 'b', role: 'b', type: 'node' },
        { id: 'b', role: 'b', type: 'node' }
      ]);
    });
  ...
  describe('#multipolygon', function () {
    ...
    specify('single polygon with single multi-way inner', function () {
      ...
      expect(r.multipolygon(graph)).to.eql([
        [
          [a.loc, b.loc, c.loc, a.loc],
          [d.loc, e.loc, f.loc, d.loc]
        ]
