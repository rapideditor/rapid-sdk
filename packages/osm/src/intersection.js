  ...
  intersection.turns = function (fromWayId, maxViaWay) {
    ...
    function step(entity, currPath, currRestrictions, matchedRestriction) {
      ...
      if (entity.type === 'node') {
          ...
          for (j = 0; j < currRestrictions.length; j++) {
            ...
            if (t.id === way.id) {
              ...
              if (v.length === 1 && v[0].type === 'node') {
                ...
                matchesViaTo =
                  v[0].id === entity.id &&
                  ((matchesFrom && currPath.length === 2) || (!matchesFrom && currPath.length > 2));
              } else {
                // match all VIA ways
                var pathVias = [];
        ...
        if (
          keyVertexIds.indexOf(n2.id) !== -1 && // key vertex..
...
export function osmInferRestriction(graph, turn, projection) {
  ...
  var angle =
    ((geoAngle(fromVertex, fromNode, projection) - geoAngle(toVertex, toNode, projection)) * 180) /
    Math.PI;
  ...
  if (
    ...
    fromOneWay &&
    toOneWay &&
    turn.from.vertex !== turn.to.vertex
  ) {
